# Fit PP process to ALCES and plot using Guthrie (2006)
# NOTE: You must have sourced 001_Analyse_Guthrie_Pleistocene_c14_Dates.R beforehand to get data


# Fit PP model to the alces data
alces_PP_fit_output <- PPcalibrate(
  rc_determinations = Alces14C$C14age,
  rc_sigmas = Alces14C$C14sig,
...
out_file_name <- paste("output/PleistoceneMegafauna/FitPP_Alces_PosteriorMean_Prior_",
...
alces_PP_mean_fit <- PlotPosteriorMeanRate(alces_PP_fit_output,
...
par(new = TRUE,
    ...
    mar = c(5, 4.5, 4, 2) + 0.1,
    las = 1)
xlim <- rev(range(alces_PP_mean_fit$calendar_age_BP))
ylim <- c(0, 3 * max(alces_PP_mean_fit$rate_mean))
plot(
  NULL,
  NULL,
  type = "n",
...
text(x = 21000, y = 0.25 *max(ylim), labels = "Alces", col = "magenta", cex = 2 )
...
out_file_name <- paste("output/PleistoceneMegafauna/FitPP_Alces_Locations_Changepoints_Prior_",
                       prior_n_internal_changepoints_lambda,
                       "_Internal_Changes", sep = "")
...
par(mgp = c(3, 0.7, 0),
    ...
    mar = c(5, 4.5, 4, 2) + 0.1,
    las = 1)
PlotPosteriorChangePoints(alces_PP_fit_output, n_changes = c(4,5, 6))
par(new = TRUE)
xlim <- rev(range(alces_PP_mean_fit$calendar_age_BP))
ylim <- c(0, 3 * max(alces_PP_mean_fit$rate_mean))
plot(
...
out_file_name <- paste("output/PleistoceneMegafauna/FitPP_Alces_Number_Changepoints_Prior_",
                       prior_n_internal_changepoints_lambda,
                       "_Internal_Changes", sep = "")
...
PlotNumberOfInternalChanges(alces_PP_fit_output)
